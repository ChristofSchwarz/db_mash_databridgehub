            ...
            functions.qrsCall('GET', config.qrsUrl + "app/full?filter=" + filter
                ...
                .then(function (appList) {
                    ...
                    appList.forEach(function (app) {
                        ...
                        $('#' + app.id).click(function () {
                            ...
                            //     })
                            //     .finally(function () {
                            //         // if querystring "from" is provided make selection in "myapplist" dropdown
                            //         if (location.search.indexOf("from=") > -1 && usedQuery.indexOf('from') == -1) {
                            //             const queryFromId = location.search.split('from=')[1].split('&')[0];
                            //             usedQuery.push('from');
                            //             console.log('Querystring from=' + queryFromId);
                            //             if ($('#myappslist option[value="' + queryFromId + '"]').length > 0) {
                            //                 // select option in dropdown
                            //                 $('#refreshfrommyapp').addClass('lui-button--info');
                            //                 $('#myappslist').show();
                            //                 $('#myappslist').val(queryFromId);
                            //                 $('#btn_replace').attr("disabled", false);
                            //             } else {
                            //                 console.log('no such option:', '#myappslist option[value="' + queryFromId + '"]');
                            //             }
                            //         }
                            //     })
                ...
                .finally(function () {
                    ...
                    if (location.search.indexOf("app=") > -1 && usedQuery.indexOf('app') == -1) {
                        ...
                        usedQuery.push('app');
                        if ($('#' + queryAppId).length > 0 && dataConnectionExists) {
